"""
AI Cost Optimizer - Prompt Optimizer (瘦身)
...
def optimize_messages(messages: List[Dict], enabled: bool = True) -> Tuple[List[Dict], int]:
    ...
    try:
        ...
        # 粗略估算: 1 token ≈ 4 chars (英文) / 2 chars (中文)
        # 取中间值 3 chars/token
        tokens_saved = max(0, (original_chars - optimized_chars) // 3)

        if tokens_saved > 0:
            logger.info(f"Prompt optimization: {original_chars} → {optimized_chars} chars, ~{tokens_saved} tokens saved")

        return optimized, tokens_saved
    except Exception as e:
        logger.error(f"optimize_messages error: {e}")
        return messages, 0
