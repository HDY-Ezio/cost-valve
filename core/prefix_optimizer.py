"""
AI Cost Optimizer - Prefix Optimizer
请求前缀优化：自动整理请求结构，最大化命中上游 LLM 的 prompt cache
...
def optimize_prefix(messages: List[Dict]) -> Tuple[List[Dict], int]:
    ...
    try:
        ...
        original_token_estimate = _estimate_tokens(messages)
        ...
        # Step 4: 确保 system prompt 一致性 hash（用于前端标记）
        optimized = ordered

        optimized_token_estimate = _estimate_tokens(optimized)

        # 节省的 token = 前缀优化后能被上游缓存命中的部分
        # 由于我们无法精确知道上游缓存了什么，保守估算：
        # system prompt + 历史对话部分 = 可缓存前缀
        cacheable_prefix_tokens = _estimate_cacheable_prefix(optimized)

        saved = max(0, cacheable_prefix_tokens)

        if saved > 0:
            logger.info(f"Prefix optimization: ~{saved} tokens in cacheable prefix")

        return optimized, saved

    except Exception as e:
        logger.error(f"optimize_prefix error: {e}")
        return messages, 0
